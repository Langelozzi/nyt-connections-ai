""" Basic BERT AGGLO evaluator: uses least confidence difference for guess"""
...
def perform_clustering(embeddings, num_clusters):
    ...
    clustering = AgglomerativeClustering(
        n_clusters=num_clusters, metric="cosine", linkage="average"
    )
...
def cluster_words_with_least_confidence_range(words, embeddings, num_clusters, group_size=4):
    """Cluster words into groups of exactly `group_size` words with least confidence range."""
    ...
    # Calculate confidence range for each cluster and sort by range
    sorted_clusters = sorted(
        clusters.values(),
        key=lambda group: max([score for _, score in group]) - min([score for _, score in group])  # Sort by range
    )
...
def bert_agglomerative_predictor_least_confidence_range(input_words: list[str]) -> list[list[str]]:
    """Predict groups of 4 words using BERT Agglomerative Clustering (least confidence range)."""
    ...
    while len(remaining_words) > 4:
        # Cluster remaining words with least confidence range
        clusters = cluster_words_with_least_confidence_range(remaining_words, remaining_embeddings, num_clusters)

        # Select the guess with the least confidence range (first cluster in sorted list)
...
def main():
    ...
    # Define predictor function (least confidence range version)
    predictor_func = bert_agglomerative_predictor_least_confidence_range
