...
def compute_phrase_embedding(phrase, word_vectors, fallback_embedding):
    ...
    for word in words:
        ...
        elif " " in word:  # Handle multi-term words
            print(f"Phrase '{word}' not found in embeddings, computing phrase embedding.")
            embeddings.append(compute_phrase_embedding(word, word_vectors, fallback_embedding))
        else:
    ...
    if embeddings:
        ...
        return fallback_embedding  # Fallback in case all words are missing
