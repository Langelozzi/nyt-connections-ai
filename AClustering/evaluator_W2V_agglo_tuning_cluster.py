...
def main():
    ...
    # Define metrics and linkages to tune
    metrics = ['euclidean', 'l1', 'l2', 'manhattan', 'cosine']
    linkages = ['single', 'complete', 'average']

    # Iterate through all metric and linkage combinations
    for metric in metrics:
        for linkage in linkages:
            if linkage == 'ward':  # Skip ward as it's incompatible with non-euclidean metrics
                continue

            print(f"Evaluating with metric='{metric}' and linkage='{linkage}'")

            # Define predictor function with current parameters
