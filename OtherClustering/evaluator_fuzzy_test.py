...
def perform_fuzzy_clustering(embeddings, num_clusters):
    """Perform Fuzzy C-Means clustering using scikit-fuzzy."""
    normalized_embeddings = normalize_embeddings(embeddings).T  # Transpose for scikit-fuzzy

    # Perform fuzzy clustering
    cntr, u, _, _, _, _, _ = fuzz.cluster.cmeans(
        normalized_embeddings, num_clusters, m=2.0, error=0.005, maxiter=1000, init=None
    )

    # Assign each point to the cluster with the highest membership score
    cluster_labels = np.argmax(u, axis=0)

    return cluster_labels, u


def cluster_words_fuzzy(words, embeddings, num_clusters, group_size=4):
    """Cluster words into groups using Fuzzy C-Means Clustering."""
    cluster_labels, memberships = perform_fuzzy_clustering(embeddings, num_clusters)
...
def word2vec_fuzzy_predictor(input_words: list[str], word_vectors) -> list[list[str]]:
    """Predict groups of 4 words using Word2Vec and Fuzzy C-Means Clustering."""
    ...
    while len(remaining_words) > 4:
        ...
        groups = cluster_words_fuzzy(remaining_words, remaining_embeddings, num_clusters)
...
def main():
    ...
    print("Evaluating Fuzzy C-Means Clustering")

    # Define predictor function with the loaded Word2Vec model
    def predictor_func(input_words):
        return word2vec_fuzzy_predictor(input_words, word_vectors)
